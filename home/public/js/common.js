...
function initializeNationalitySearch(inputId, listId, arrivalId, arrivalListId = null, independentCities = true) {
    ...
    if (!nationalityInput || !nationalityList || !arrivalElement) {
        console.log(`Skipping ${inputId} - elements not found on this page`);
        return false;
    }

    let selectedCountryCities = [];
    const isArrivalSelect = arrivalElement.tagName === 'SELECT';
        ...
        items.forEach(country => {
            ...
            item.addEventListener('click', function () {
                ...
                if (country.cities) {
                    ...
                    if (isArrivalSelect) updateArrivalPlacesForForm(country.cities, arrivalElement);
                }
            });
    ...
    if (!isArrivalSelect && arrivalList) {
