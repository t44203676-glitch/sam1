...
// ─────────────────────────────────────────────────────
// 8. دالة الصورة الشخصية (صور المرفوعة)
//    تتحقق من مكان الصورة وتعيد المسار الصحيح أو Fallback
// ─────────────────────────────────────────────────────
if (!function_exists('getProfilePhotoUrl')) {
    function getProfilePhotoUrl(?string $rawPath, ?string $fallback = null): string {
        $fallback = $fallback ?? getInquiryAsset('images/default-avatar.png');

        if (empty($rawPath) || $rawPath === '---') {
            return $fallback;
        }

        // إذا كان رابط كامل (http/https) أعده مباشرة
        if (str_starts_with($rawPath, 'http')) {
            return $rawPath;
        }

        // مسار عبر image_server.php (بيئة محلية)
        if (defined('UPLOADS_BASE_URL') && defined('UPLOADS_PHYSICAL_PATH')) {
            $cleanPath = ltrim($rawPath, '/');
            $physPath  = UPLOADS_PHYSICAL_PATH . $cleanPath;
            if (file_exists($physPath)) {
                return UPLOADS_BASE_URL . urlencode($cleanPath);
            }
        }

        // مسار نسبي من BASE_URL
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . ltrim($rawPath, '/'))) {
            return BASE_URL . ltrim($rawPath, '/');
        }
