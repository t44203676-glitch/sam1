...
/**
 * دالة عرض شارة الحالة (Status Badge)
 * تُرجع HTML لشارة ملونة حسب حالة الطلب
 */
if (!function_exists('get_status_badge')) {
    function get_status_badge($status) {
        $status = trim($status ?? 'تمت الموافقة');

        $badgeStyles = [
            'تمت الموافقة'   => ['bg' => '#28a745', 'icon' => '✔'],
            'موافق عليه'     => ['bg' => '#28a745', 'icon' => '✔'],
            'مرفوض'          => ['bg' => '#dc3545', 'icon' => '✘'],
            'قيد المراجعة'   => ['bg' => '#ffc107', 'icon' => '⏳', 'color' => '#000'],
            'معلق'           => ['bg' => '#6c757d', 'icon' => '⏸'],
        ];

        $style = $badgeStyles[$status] ?? ['bg' => '#17a2b8', 'icon' => 'ℹ'];
        $color = $style['color'] ?? '#fff';

        return '<span style="display:inline-block;padding:4px 14px;border-radius:20px;font-size:14px;font-weight:bold;background:' . $style['bg'] . ';color:' . $color . ';">'
             . $style['icon'] . ' ' . htmlspecialchars($status)
             . '</span>';
    }
}
