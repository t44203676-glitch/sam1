<?php
/**
 * admin/serves/views/profession_changes_view.php
 */
...
if ($pdo) {
    try {
        ...
        $stmt_partners = $pdo->prepare("SELECT * FROM related_data WHERE profession_change_id = ?");
...
{
    $is_numeric = ($name === 'national_id' || $name === 'service_number' || $name === 'serial_number' || $name === 'issuance_number' || $name === 'sponsor_id' || $name === 'export_number');
    $display_value = $is_numeric ? toWesternDigits($value) : $value;
    $val = !empty($display_value) ? htmlspecialchars($display_value) : '---';

    // Build the edit-mode field
    if (!empty($options)) {
    ...
    <form id="updateRequestForm" enctype="multipart/form-data">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom flex-wrap gap-3 bg-white p-3 rounded shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 text-dark fw-bold">تغيير المهنة</h4>
...
?>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-primary text-white py-3 border-0">
                <h5 class="mb-0 fs-6"><i class="fas fa-file-invoice me-2"></i> بيانات الطلب والكفيل</h5>
                </div>
...
render_service_field('رقم الهوية', 'national_id', $request['national_id']);
render_service_field('رقم الجوال', 'phone', $request['phone']);
render_service_field('اسم الكفيل', 'sponsor_name', $request['sponsor_name']);
render_service_field('اسم المنشأة', 'establishment_name', $request['establishment_name']);
render_service_field('رقم هوية الكفيل', 'sponsor_id', $request['sponsor_id']);
render_service_field('مكتب الجوازات', 'emirate', $request['emirate'], 'text', [
...
if (!in_array($_SESSION['user_type'], ['موظف', 'Employee'])) {
    render_service_field('رقم المصدر', 'service_number', $request['service_number'] ?? $request['export_number']);
}
render_service_field('تاريخ آخر تعديل (هجري)', 'hijri_date', $request['hijri_date']);
...
?>
        ...
        <div class="card border-0 shadow-sm mb-5">
            <div class="card-header bg-info text-white py-3 border-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fs-6"><i class="fas fa-users-cog me-2"></i> بيانات الأفراد وتعديل المهن</h5>
                    ...
                    <table class="table table-hover align-middle mb-0" id="partners-table">
                        <thead class="bg-light text-muted small">
                            <tr>
                                ...
                                <th class="py-3">المهنة السابقة</th>
                                <th class="py-3">المهنة الجديدة</th>
...
foreach ($partners as $partner):
    $serialStr = str_pad($count, 2, '0', STR_PAD_LEFT);
    $serialAr = toArabicDigits($serialStr);
?>
                             <tr data-id="<?php echo $partner['id']; ?>">
                                <td class="px-4 py-3 fw-bold text-muted small"><?php echo $serialAr; ?></td>
                                <td class="py-3 fw-bold text-primary" data-field-name="full_name"><?php echo htmlspecialchars($partner['full_name'] ?? '---'); ?></td>
                                <td data-field-name="national_id"><?php echo htmlspecialchars(toWesternDigits($partner['national_id'] ?? '---')); ?></td>
                                <td data-field-name="old_profession"><?php echo htmlspecialchars($partner['old_profession'] ?? '---'); ?></td>
