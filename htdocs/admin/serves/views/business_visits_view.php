<?php
/**
 * admin/serves/views/business_visits_view.php
 */
...
if ($pdo) {
    try {
        ...
        $stmt_partners = $pdo->prepare("SELECT * FROM related_data WHERE business_visit_id = ?");
    ...
    <form id="updateRequestForm" enctype="multipart/form-data">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom flex-wrap gap-3 bg-white p-3 rounded shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 text-dark fw-bold">زيارات الأعمال</h4>
...
render_service_field('رقم التأشيرة', 'visa_no', $request['visa_no']);
render_service_field('تاريخ الإصدار', 'issue_date', $request['issue_date'], 'date');
render_service_field('صالحة لغاية', 'valid_until', $request['valid_until'], 'date');
render_service_field('مدة الإقامة', 'duration_of_stay', $request['duration_of_stay']);
render_service_field('نوع التأشيرة', 'visa_type', $request['visa_type']);
render_service_field('عدد مرات الدخول', 'entry_type', $request['entry_type']);
render_service_field('رقم الإقامة المصدرة', 'visa_residence_no', $request['visa_residence_no']);
render_service_field('تاريخ الانتهاء', 'expiry_date', $request['expiry_date'], 'date');
?>
                </div>
            </div>
        </div>
            ...
            <div class="container-fluid px-2 px-md-5">
                <div class="d-flex justify-content-center gap-3" id="view-mode-actions">
                    ...
                    <?php if (($_SESSION['user_type'] === 'مدير' || $_SESSION['user_type'] === 'Root') && ($request['status'] === 'بانتظار موافقة المدير')): ?>
                        ...
                        <button type="button" class="btn btn-success px-4 rounded-pill shadow-sm" onclick="updateRequestStatus(<?php echo $id; ?>, 'تمت الموافقة', '<?php echo $table; ?>')">موافقة</button>
                        <button type="button" class="btn btn-danger px-4 rounded-pill shadow-sm" onclick="showRejectDialog(<?php echo $id; ?>, '<?php echo $table; ?>')">رفض</button>
                    <?php
endif; ?>
                </div>
