<?php
/**
 * admin/serves/views/followup_requests_view.php
 */
...
if ($pdo) {
    try {
        ...
        $stmt_partners = $pdo->prepare("SELECT * FROM related_data WHERE followup_request_id = ?");
    ...
    <form id="updateRequestForm" enctype="multipart/form-data">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom flex-wrap gap-3 bg-white p-3 rounded shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 text-dark fw-bold">التعقيب</h4>
...
?>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-primary text-white py-3 border-0">
                <h5 class="mb-0 fs-6"><i class="fas fa-user-check me-2"></i> البيانات الأساسية لمقدم الطلب</h5>
                </div>
...
render_service_field('تاريخ الهجري', 'hijri_date', $request['hijri_date'] ?? '');
render_service_field('تاريخ الإصدار', 'last_modified_date', $request['last_modified_date'] ?? '');
render_service_field('ملاحظات', 'remarks', $request['remarks'] ?? '');
?>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-5">
            <div class="card-header bg-info text-white py-3 border-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fs-6"><i class="fas fa-stream me-2"></i> بيانات المتابعة الإضافية</h5>
                    ...
                    <table class="table table-hover align-middle mb-0" id="partners-table">
                        <thead class="bg-light text-muted small">
                            <tr>
                                ...
                                <th class="py-3">تاريخ الارسال للبنك</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr data-id="<?php echo $partner['id']; ?>">
                                <?php if (!in_array($_SESSION['user_type'], ['موظف', 'Employee'])): ?>
                                    <td class="px-4 py-3 fw-bold text-muted" data-field-name="serial_number"><?php echo htmlspecialchars(toWesternDigits($request['serial_number'] ?? '---')); ?></td>
                                <?php
    endif; ?>
                                <td class="py-3" data-field-name="job_category"><?php echo htmlspecialchars($partner['job_category'] ?? '---'); ?></td>
                                <td data-field-name="nationality"><?php echo htmlspecialchars($partner['nationality'] ?? '---'); ?></td>
                                <td data-field-name="visa_type"><?php echo htmlspecialchars($partner['visa_type'] ?? 'تعقيب وجوازات'); ?></td>
                                <td data-field-name="country"><?php echo htmlspecialchars($partner['country'] ?? $partner['arrival_place'] ?? '---'); ?></td>
                                <td data-field-name="bank_file_number"><?php echo htmlspecialchars(toWesternDigits('0')); ?></td>
                                <td data-field-name="bank_send_date">---</td>
                            </tr>
                            <?php
endforeach; ?>
                        </tbody>
...
function enableEditMode() {
    ...
    document.querySelectorAll('#partners-table tbody tr').forEach(row => {
        row.querySelectorAll('td[data-field-name]').forEach(cell => {
            ...
            if (field === 'serial_number') return; // Read-only
