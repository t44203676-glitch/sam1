<?php
/**
 * admin/serves/views/recruitment_requests_view.php
 */
...
{
    $is_numeric = ($name === 'national_id' || $name === 'service_number' || $name === 'serial_number' || $name === 'issuance_number' || $name === 'sponsor_id' || $name === 'export_number' || $name === 'bank_file_number');
    ...
    <form id="updateRequestForm" enctype="multipart/form-data">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom flex-wrap gap-3 bg-white p-3 rounded shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 text-dark fw-bold">الاستقدام</h4>
...
render_service_field('مكتب الاستقدام', 'emirate', $request['emirate'] ?? '', 'text', [
...
if (!in_array($_SESSION['user_type'], ['موظف', 'Employee'])) {
    render_service_field('رقم المصدر', 'service_number', !empty($request['service_number']) ? $request['service_number'] : ($request['export_number'] ?? ''));
}
render_service_field('تاريخ الإصدار (هجري)', 'hijri_date', $request['hijri_date'] ?? '');
...
render_service_field('تاريخ الموافقة', 'approval_date', $request['approval_date'] ?? '');
?>
                </div>
            </div>
        </div>
                        ...
                        <thead class="bg-light text-muted small">
                            <tr>
                                ...
                                <?php if (!in_array($_SESSION['user_type'], ['موظف', 'Employee'])): ?>
                                    <th class="py-3">الرقم التسلسلي</th>
                                <?php
endif; ?>
                                ...
                                <th class="py-3">حالة التأشيرة</th>
...
foreach ($partners as $partner): ?>
                            <tr data-id="<?php echo $partner['id']; ?>">
                                ...
                                <?php if (!in_array($_SESSION['user_type'], ['موظف', 'Employee'])): ?>
                                    <td data-field-name="serial_number"><?php echo htmlspecialchars(toWesternDigits($request['serial_number'] ?? '---')); ?></td>
                                <?php
    endif; ?>
                                ...
                                <td data-field-name="bank_file_number"><?php echo htmlspecialchars(toWesternDigits($partner['bank_file_number'] ?? '0')); ?></td>
