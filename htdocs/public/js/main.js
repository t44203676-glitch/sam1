...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Global listener to convert Eastern Arabic numerals (٠-٩) to English numerals (0-9) as the user types
    document.addEventListener('input', function (e) {
        if (e.target && (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA')) {
            // We only want to auto-convert for text-based inputs
            if (['text', 'tel', 'textarea', 'search'].includes(e.target.type)) {
                const val = e.target.value;
                if (!val) return;

                const converted = val.replace(/[٠-٩]/g, function (d) {
...
function deleteRelatedItem(formType, requestId, itemId, buttonElement) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const data = { formType, requestId, itemId };
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        if (row) {
                            ...
                            // If table is now empty, add back the placeholder row
                            if (tableBody && tableBody.rows.length === 0) {
                                const colCount = table.querySelectorAll('thead th').length || 4;
                                tableBody.innerHTML = `<tr><td colspan="${colCount}" class="text-center placeholder-row">لا توجد بيانات مضافة</td></tr>`;
                            }
                        }
                    } else {
...
function getRowTemplate(item, formType, tableBody) {
    ...
    if (formType === 'marriage') {
        return `
            <td data-label="الاسم الكامل">${item.full_name || ''}</td>
            <td data-label="رقم الجواز/الإقامة">${item.passport_number || '---'}</td>
            <td data-label="فئة المهنة">${item.job_category || '---'}</td>
            <td data-label="نوع التصريح">${item.permit_type || '---'}</td>
            <td data-label="الجنسية">${item.nationality || ''}</td>
            <td data-label="مكان القدوم">${item.country || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
        `;
    }
    // Template for Family Visit form
        ...
        return `
            <td data-label="الاسم الكامل">${item.full_name || ''}</td>
            <td data-label="صلة القرابة">${item.relationship || '---'}</td>
            <td data-label="العمر">${item.age || '---'}</td>
            <td data-label="المدة">${item.duration || item.duration_of_stay || '---'}</td>
            <td data-label="المهنة">${item.job_category || '---'}</td>
            <td data-label="رقم التأشيرة">${item.visa_no || '---'}</td>
            <td data-label="نوع التأشيرة">${item.visa_type || '---'}</td>
            <td data-label="تاريخ الإصدار">${item.issue_date || '---'}</td>
            <td data-label="تاريخ الانتهاء">${item.valid_until || item.expiry_date || '---'}</td>
            <td data-label="رقم الجواز">${item.passport_number || '---'}</td>
            <td data-label="الجنسية">${item.nationality || '---'}</td>
            <td data-label="جهة القدوم">${item.country || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
        `;
    }
    // Template for Business Visit form
        ...
        return `
            <td data-label="الاسم">${item.full_name || ''}</td>
            <td data-label="تاريخ الميلاد">${item.birth_date || '---'}</td>
            <td data-label="رقم الجواز">${item.passport_number || '---'}</td>
            <td data-label="الجنسية">${item.nationality || '---'}</td>
            <td data-label="جهة القدوم">${item.country || item.arrival_place || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
        `;
    }
    ...
    // Template for Runaway Cancellation form
        ...
        return `
            <td data-label="الاسم">${item.full_name || ''}</td>
            <td data-label="رقم الجواز">${item.passport_number || '---'}</td>
            <td data-label="المهنة القديمة">${item.old_profession || '---'}</td>
            <td data-label="المهنة">${item.job_category || '---'}</td>
            <td data-label="تاريخ الإصدار">${item.iqama_issue_date || '---'}</td>
            <td data-label="تاريخ الانتهاء">${item.iqama_expiry_date || '---'}</td>
            <td data-label="الجنسية">${item.nationality || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
        `;
    }
    // Template for Profession Change form
        ...
        return `
            <td data-label="الاسم">${item.full_name || ''}</td>
            <td data-label="رقم الهوية">${item.national_id || '---'}</td>
            <td data-label="رقم الجواز">${item.passport_number || '---'}</td>
            <td data-label="المهنة القديمة">${item.old_profession || '---'}</td>
            <td data-label="المهنة الجديدة">${item.job_category || '---'}</td>
            <td data-label="تاريخ الإصدار">${item.iqama_issue_date || '---'}</td>
            <td data-label="تاريخ الانتهاء">${item.iqama_expiry_date || '---'}</td>
            <td data-label="الجنسية">${item.nationality || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
    ...
    // Template for Labor form
    else if (formType === 'labor') {
        return `
            <td data-label="المهنة">${item.job_category || ''}</td>
            <td data-label="الجنسية">${item.nationality || ''}</td>
            <td data-label="جهة القدوم">${item.country || item.arrival_place || '---'}</td>
    ...
    // Template for Followup form
    else if (formType === 'followup') {
        const serialEl = document.getElementById('main-serial-number');
        const serialNumber = serialEl ? serialEl.innerText.trim() : (document.querySelector('.related-items-table tbody tr td[data-label="الرقم التسلسلي"]') ? document.querySelector('.related-items-table tbody tr td[data-label="الرقم التسلسلي"]').innerText : '---');
        return `
            <td data-label="الرقم التسلسلي">${serialNumber}</td>
            <td data-label="فئة المهنة">${item.job_category || '---'}</td>
            <td data-label="الجنسية">${item.nationality || '---'}</td>
            <td data-label="النوع">${item.visa_type || 'تعقيب وجوازات'}</td>
            <td data-label="مكان القدوم">${item.country || item.arrival_place || '---'}</td>
            <td data-label="الحالة">${item.status || '---'}</td>
            <td data-label="رقم الملف المرسل للبنك">0</td>
            <td data-label="تاريخ الارسال للبنك">---</td>
            ${actionsCell}
        `;
    }
    // Template for Recruitment form
    ...
    // Template for Civil Affairs form
    else if (formType === 'civil_affairs') {
        const serialNumber = '11' + String(item.id % 100000000).padStart(8, '0');
        const mainServiceType = document.querySelector('[name="service_type"]') ? document.querySelector('[name="service_type"]').value : 'أحوال مدنية';
        return `
            <td data-label="رقم المعاملة">${serialNumber}</td>
            <td data-label="المستفيد">${item.relationship || item.job_category || '---'}</td>
            <td data-label="نوع المعاملة">${item.service_type || mainServiceType}</td>
            <td data-label="تاريخ الحجز">${item.appointment_date || '---'}</td>
            ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
        `;
    }
    ...
    return `
        <td data-label="الاسم">${item.full_name || ''}</td>
        <td data-label="الرقم">${item.passport_number || ''}</td>
        <td data-label="المهنة">${item.relationship || item.job_category || ''}</td>
        <td data-label="الجنسية">${item.nationality || ''}</td>
        ${(tableBody.closest('table') && tableBody.closest('table').querySelector('th:nth-child(5)') && tableBody.closest('table').querySelector('th:nth-child(5)').innerText.includes('القدوم')) ? `<td data-label="القدوم">${item.country || item.arrival_place || ''}</td>` : ''}
        ${actionsCell.replace('<td class="text-center">', '<td data-label="إجراءات" class="text-center">')}
    `;
}
...
function addRowToTable(item, formType) {
    ...
    // Remove placeholder if it's the only row there
...
function populateFormForEdit(itemData, formType) {
    ...
    const formContainer = document.querySelector(`#related${formType.charAt(0).toUpperCase() + formType.slice(1)}Form, #relatedPersonForm, #relatedPartnerForm, #relatedMemberForm, #relatedVisitorForm, #relatedLaborForm`);
    if (!formContainer) {
    ...
    for (const key in itemData) {
        ...
        if (input) {
            if (input.type !== 'file' && input.type !== 'submit') {
                // If the value is empty, null, or '---', check for a default value from data attribute
...
function resetRelatedItemForm(formElement) {
    ...
    // Restore default values from data attributes
    formElement.querySelectorAll('[data-default-value]').forEach(el => {
