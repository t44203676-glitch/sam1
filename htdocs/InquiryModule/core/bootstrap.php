...
//    تستخدم المنطق المركزي: استخراج اسم الملف + UPLOADS_BASE_URL
// ─────────────────────────────────────────────────────
if (!function_exists('getProfilePhotoUrl')) {
    function getProfilePhotoUrl(?string $rawPath, ?string $fallback = null): string {
        ...
        // استخراج اسم الملف فقط (إزالة أي مسارات قديمة مثل uploads/personal_photos/)
        $filename = basename($rawPath);

        // بناء الرابط النهائي من UPLOADS_BASE_URL المركزي
        if (defined('UPLOADS_BASE_URL')) {
            return rtrim(UPLOADS_BASE_URL, '/') . '/' . ltrim($filename, '/');
        }

        // Fallback: مسار نسبي من BASE_URL
        return (defined('BASE_URL') ? rtrim(BASE_URL, '/') : '') . '/uploads/' . $filename;
    }
}
